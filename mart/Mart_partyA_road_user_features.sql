-- 針對與行人直接相關的車禍案件，分析第一肇事者的(可能是行人也可能是駕駛人)的用路行為
-- 1. 先串接出第一肇事者所需的欄位：個人身份特徵與當下用路行為。然後留下主要肇因關係有包含人的。
CREATE OR REPLACE VIEW v_sq1_main_sub_human AS
...
CREATE OR REPLACE VIEW v_sq1_main_sub_human_grouped AS
	SELECT 
...
CREATE TABLE IF NOT EXISTS mart_partyA_road_user_features AS
	WITH partitioned_avgs AS (
		SELECT 
			...
			FROM v_sq1_main_sub_human_grouped
				WHERE age_grouped != "未知" AND accident_type_major_grouped = "人與車"
		  )
	SELECT * FROM partitioned_avgs;

DROP VIEW v_sq1_main_sub_human;
DROP VIEW v_sq1_main_sub_human_grouped;